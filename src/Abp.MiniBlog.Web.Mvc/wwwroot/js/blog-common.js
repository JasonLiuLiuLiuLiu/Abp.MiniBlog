...
function GetMeta(n) {
    for (var i = document.getElementsByTagName("meta"), t = 0; t < i.length; t++)
        if (i[t].name.toLowerCase() == n) return i[t].content;
    return ""
}
...
function codeHighlight() {
    cb_entryId > 6251266 && !isMarkdown && typeof Prism == "undefined" && typeof hljs == "undefined" && $("pre code").length && ($("head").append('<link rel="stylesheet" type="text/css" href="https://common.cnblogs.com/highlight/styles/default.min.css?id=20190113">'), $.getScript("https://common.cnblogs.com/highlight/9.12.0/highlight.min.js", function() {
...
function loadNewsAndKb() {
    ...
    $("#under_post_kb").is(":visible") && $.ajax({
        url: "/mvc/Blog/UnderPostNews.aspx",
        type: "get",
        dataType: "text",
        success: function(n) {
...
function loadBlogCalendar(n) {
    (typeof currentBlogId == "undefined" || currentBlogId != 193295 || typeof cb_entryId == "undefined") && $.ajax({
...
function loadBlogSideColumn() {
    (typeof currentBlogId == "undefined" || currentBlogId != 193295 || typeof cb_entryId == "undefined") && $("#blog-sidecolumn").length && $.ajax({
...
function blogCommentManager() {
    ...
    var n = function(n) {
        ...
        i = function() {
            if (!allowComments || !enablePostBottom()) {
    ...
    this.PostNewComment = function() {
        ...
        $.ajax({
            ...
            timeout: 3e4,
            success: function(n) {
                if (n) n.IsSuccess ? (ShowCommentMsg("感谢您的回复:) 服务器端执行耗时" + n.Duration + "毫秒"), $("#tbCommentBody").val(""), $("#divCommentShow").html($("#divCommentShow").html() + n.Message), cb_mathjax_render("#divCommentShow")) : ShowCommentMsg(n.Message), $("#btn_comment_submit").removeAttr("disabled");
                else ShowCommentMsg("抱歉！评论提交失败！请与管理员联系(contact@cnblogs.com)。"), $("#btn_comment_submit").removeAttr("disabled")
            },
            error: function(n, t) {
    ...
    this.UpdateComment = function() {
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf8",
            success: function(t) {
                if (t) t.IsSuccess ? (ShowCommentMsg("修改成功"), $("#comment_body_" + n.commentId).html(t.Message), commentManager.ResetCommentBox()) : ShowCommentMsg(t.Message);
                else ShowCommentMsg("抱歉！评论修改失败！请与管理员联系(contact@cnblogs.com)。")
            },
            error: function(n) {
...
function GetQuoteComment(n) {
    var i = $("#comment_body_" + n + " div.syntaxhighlighter"),
        t, r;
    $(i).length && $(i).remove();
...
function showImages(n) {
    if (cb_entryId > 4123456) {
        ...
        $(t).each(function() {
            var n, t;
            this.src.indexOf(".qpic.cn") > 0 && (n = $(this).attr("data-src"), typeof n != typeof undefined && n.length > 10 && (this.src = n));
            this.src.indexOf(".jianshu.io") > 0 && (t = $(this).attr("data-original-src"), typeof t != typeof undefined && t.length > 10 && (this.src = t))
        })
    }
}
...
function markdown_highlight() {
    typeof Prism == "undefined" && $("#cnblogs_post_body pre code").each(function(n, t) {
...
function getRecommendedList() {
    ...
    $.ajax({
        ...
        success: function(n) {
            var i, t;
            if (n && n.length >= 5) {
                for (i = '<div class="itnews c_ad_block"><b>相关博文：<\/b><br>', t = 0; t < n.length && t < 5; t++) i += '·  <a href="' + n[t].url.replace("http://", "https://") + '" target="_blank" onclick="clickRecomItmem(' + n[t].itemId + ')">' + n[t].title + "<\/a><br />";
                $("#under_post_news").html(i)
            } else $("#under_post_news").hide()
        },
        error: function() {
            $("#under_post_news").hide()
        }
    })
}
...
function enablePostBottom() {
    return cb_blogId == 193295 ? !1 : !0
}
...
function deliverC1() {
    ...
    }) : $.getJSON("https://a1.cnblogs.com/group/C1", function(n) {
        if (n) {
            var t = n.C1;
            t.indexOf("gitee.com") > 0 && (t = t.replace("ga('send', 'event', 'Link', 'click', 'C1');", "ga('send', 'event', 'Link', 'click', 'C1-default-gitee');"));
            $("#cnblogs_c1").html(t)
        }
    })
}
...
function deliverC2() {
    canShowAdsense() ? screen.width < 500 ? deliverC2Mobile() : googletag.cmd.push(function() {
        googletag.display("div-gpt-ad-1539008685004-0")
    }) : $("#cnblogs_c2").hide()
}

function deliverC2Mobile() {
    $("#cnblogs_c2").html('        <script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"><\/script>        <ins class="adsbygoogle"             style="display:inline-block;width:320px;height:50px"             data-ad-client="ca-pub-4210569241504288"             data-ad-slot="4563868684"><\/ins>        <script>        (adsbygoogle = window.adsbygoogle || []).push({});        <\/script>')
}
...
function canShowAdsense() {
    ...
    return (n = $("#cnblogs_post_body"), n.length && (i = n.text(), i.length < 300)) ? !1 : (r = [1489405, 1873020, 3658314, 3349843, 3120490, 2131583, 5601171, 7783178, 2117105, 4315037, 3343865, 5106828, 9943394, 1512482, 8615503, 8495009, 8481402, 3873909, 8414234, 1718087, 6021462, 3925643, 6435456, 9242918, 6666860], r.includes(cb_entryId)) ? !1 : !0
}
...
$.fn.extend({
    ...
    parseHtml: function(n) {
        ...
        if (t) this.get(0).focus(), t.selection.createRange().collapse, this.get(0).document.selection.createRange().text = n;
